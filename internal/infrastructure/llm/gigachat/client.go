...
2. Выполниnm дополнительный запрос к бд (чтобы понять какие данные там лежат и что хочет пользователь) и указать 'need_query: true', а в sql запрос который нужно выполнить. Следующим сообщением я верну тебе результат запроса.
После того как я отпралю тебе результат с БД выполни следующие этапы:
	...
	запрос то он должен быть готов к выполнению и возвращать колонки, необходимые для построения графика (например, для line — колонка со временем/осью X и числовая колонка; для pie — категориальная и числовая; для histogram — интервал или границы и частота).
